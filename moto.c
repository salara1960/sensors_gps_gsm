...
{
unsigned char symbol = 0, bt = 1, prn = 0, sz = 0, en = 0, fst = 1;
unsigned char pst = 0, byte = 0, pst_dht = 0, pst_hcsr = 0, pst_hcsr2 = 0, pst_enc = 0, stop_all_rk = 0;
unsigned char data_wr_w1 = 0, pst_adc = 0, id, busy = 0, first = 1, cmd_reg_5215, cmd_reg_10;
char chaka[128] = {0};
char stroka[80] = {0};
...
unsigned char wr_cmd[16], rd_cmd[16], rd_dht[16], rd_adc[16], rd_enc[16];
int res, lens, ind = 0, rd_uk = 0, rd_cnt = 9, i, j, eol = 0;
int rd_uk_dht = 0, rd_cnt_dht = 5, rd_uk_adc = 0, rd_cnt_adc = 4;
int rd_uk_hcsr = 0, rd_cnt_hcsr = 4, rd_uk_hcsr2 = 0, rd_cnt_hcsr2 = 4;
int rd_cnt_enc = 6;
struct timeval mytv;
fd_set my_read_Fds;
char buf[mem_buf];
unsigned int tmrs, tmr_w1 = 0, tmr_loop, tmr_hcsr, tmr_hcsr2, msec_def = _500ms, tmr_enc, msec_enc = _500ms;
char *endp = NULL;
char ds18_str[32];
char dht11_str[32];
int my_max_fd;
...
unsigned short enc_short, gsm_len = 0;
char stx[tmp_buf_size+1] = {0};
char stz[128];
unsigned int mtr_way = 0, enc_way_sm = 0;
unsigned char mtr_start_first = 0;

    pid_main = getpid();

    if (!(pid_fp = fopen(pid_name, "w"))) {
	memset(chaka, 0, sizeof(chaka)); 
	sprintf(chaka,"moto: unable to create pid file %s: %s\n",pid_name, strerror(errno)); printf(chaka);
	return -1;
    } else {
	fprintf(pid_fp, "%i\n", (int)pid_main);
	fclose(pid_fp);
...
//	else if (!strcmp(chaka,"convert")) convert = 1;
...
//-----------------------------------------------------------------------------------
	...
	memset(stroka, 0, 80); sprintf(stroka,"%s",DTPrn());
	sprintf(chaka,"%s START MAIN with PID %d\n",stroka,pid_main);
	write(gsm_log, chaka, strlen(chaka));
    }
	...
	memset(stroka, 0, 80); sprintf(stroka,"%s",DTPrn());
	sprintf(chaka,"%s START MAIN with PID %d\n",stroka,pid_main);
	write(fd_bug, chaka, strlen(chaka));
    }
	...
	memset(stroka, 0, 80); sprintf(stroka,"%s",DTPrn());
	sprintf(chaka,"%s START MAIN with PID %d\n",stroka,pid_main);
	write(fd_log, chaka, strlen(chaka));
    }

    if (sz) printf("size_of_pack=%d\n",sizeof(s_pack));
...
// /dev/totoro0 - moto  /dev/totoro1 - gps  /dev/totoro2 - ds18  /dev/totoro3 - dht11  /dev/totoro4 - ADC  /dev/totoro5 - HCSR  /dev/totoro7 - HCSR2
	...
	memset(chaka, 0, 80); memset(stroka, 0, 80);
...
//    ReadStop(4);//ADC
    ...
    *(unsigned char *)SANI_ADC_CHAN = tmp_chan;

    ReadStop(5);//HCSR
    ReadStop(7);//HCSR2
    ReadStop(6);//ENC

//    moto_write((unsigned char)MOTO_STOP);//write 100
...
//------------------------------   GSM   ------------------------------------------
    ...
    memset((unsigned char *)&gsm_rk[0], 0, i);

    cmd_reg_5215 = cmd_reg_10 = 255;
    for (i = 0; i < max_at; i++) {
	if (strstr((char *)commands5215[i],"AT+CREG?") != NULL) {
	    cmd_reg_5215 = (unsigned char)i; break;
	}
    }
    for (i = 0; i < max_at; i++) {
	if (strstr((char *)commands10[i],"AT+CREG?") != NULL) {
	    cmd_reg_10 = (unsigned char)i; break;
	}
    }
    *(unsigned char *)SANI_RK = N_RK;
//printf("\ncmd_reg_5215=%d cmd_reg_10=%d\n",cmd_reg_5215,cmd_reg_10);
	...
	if (RK_table[i].present) {
		...
		printf(chaka); print_rk(chaka);
	    } else {
		//fcntl(fd[i], F_SETFL, O_NONBLOCK);
		sprintf(chaka,"Open /dev/gsm%d OK\n",i);
		printf(chaka); print_rk(chaka);

//		if (i==N_RK) KT_Point(i);
		if (OneChannel) {
...
//---------------------------------------------------------------------------------
    memset((unsigned char *)&enc.way, 0, sizeof(s_enc));
    memset((unsigned char *)&one_pack.utc.chas, 0, sizeof(s_pack));
    memset((unsigned char *)&web_pack.utc.chas, 0, sizeof(s_pack));

    *(unsigned char *)SANI_W1_WR = 0;
    *(unsigned char *)SANI_W1_RD = 0;
    symbol = 0x30;
    web_uk = (unsigned char *)&one_pack;
    *(unsigned char *)SANI_MSG_TYPE = msg_type_ind;
    *(unsigned char *)SANI_MSG_TYPE_SET = msg_type_ind;
    *(unsigned char *)SANI = 0x30;
    *(unsigned char *)SANI_TYPE_POST = 0x30;
    memcpy(SANI_ENC_WAY, (unsigned char *)&enc.way, sizeof(s_enc));
    rst_dev(6, 0);//6=ENC

    init_motor();

    loops = 1;
    ind = 0; memset(buf, 0, mem_buf);
    bt = 1;
	...
	//-------------------------- post from F9 -------------------------------
	symbol = *(unsigned char *)(SANI_POST_AT);//есть ли новый пост от F9
	if (symbol == 0x31) {
	    if (!ATCOM.flag_) {
		ATCOM.flag_ = 1;
	    ...
	    *(unsigned char *)(SANI_POST_AT) = 0x30;//сбросить флаг F9
//memset(stz,0,128); sprintf(stz,"AT_POST: '%s' flag=%d\n",(char *)(SANI_AT+2),ATCOM.flag_); print_rk(stz);
	}
	//-----------------------------------------------------------------------

	symbol = *(unsigned char *)SANI_TYPE_POST;
	if (symbol==0x31) {// есть пост по смене типа сообщения у GPS
	    *(unsigned char *)SANI_TYPE_POST = 0x30;
	    //for GPS message type
	    msg_type_ind_set = *(unsigned char *)SANI_MSG_TYPE_SET;
...
//printf("after: old=%d new=%d\n",msg_type_ind,msg_type_ind_set);
	...
	if (check_delay_sec(tmr_loop)) {
		...
		*(unsigned char *)(SANI_W1_WR) = 0xAA;//цепочка 0xCC 0x44, 0xCC 0xBE
		*(unsigned char *)SANI = 0x31;
	    }
	}
	
	//-------------   motor   --------------------------------------------------------
	if (*(unsigned char *)SANI_MOTOR_POST == 0x31) {// есть пост от вэба для моторчика
	    *(unsigned char *)SANI_MOTOR_POST = 0x30;
	    memcpy((unsigned char *)&motor, SANI_MOTOR_DATA, sizeof(s_motor));
	    //if (motor.speed==100) mtr_start_first=0;
		...
		break;//soft
		    //default : moto_write((unsigned char)MOTO_STOP);//pult
	    }
	    moto_read();
	    memcpy(SANI_MOTOR_DATA, (unsigned char *)&motor, sizeof(s_motor));
	}

	symbol = *(unsigned char *)SANI;
	if (symbol == 0x31) {// есть пост от вэба
	    tmp_chan = *(unsigned char *)SANI_ADC_CHAN;
	    if (tmp_chan != sel_chan) {
		if (select_adc_chan(tmp_chan)) {
		    memset(chaka, 0, 80);
		    sprintf(chaka+strlen(chaka)," Select_adc_chan ERROR : old=%d new=%d\n",sel_chan,tmp_chan);
		    gps_log(chaka); bug_log(chaka);
		}
	    }

	    memset(ds18_str, 0, 32);
	    memset(dht11_str, 0, 32);
	    busy = 1;
/*
	    //-------------   motor   --------------------------------------------------------
	    if (*(unsigned char *)SANI_MOTOR_POST==0x31) {// есть пост от вэба для моторчика
		memcpy((unsigned char *)&motor, SANI_MOTOR_DATA, sizeof(s_motor));
		if (motor.mode == 0) i = SET_MODE_PULT;
		else if (motor.mode == 1) i = SET_MODE_SOFT;
		else i = -1;
		if (i != -1) SetModeMotor(i);
		switch (motor.mode) {
		    case 1 : moto_write(motor.speed); break;//soft
			default : moto_write((unsigned char)MOTO_STOP);//pult
		}
		moto_read();
		memcpy(SANI_MOTOR_DATA, (unsigned char *)&motor, sizeof(s_motor));
...
*/
	    ...
	    rd_cnt = 0;
	    pst = 0;
	    memset(rd_cmd, 0, 16); memset(SANI_W1_RD, 0, 16);
	    data_wr_w1 = *(unsigned char *)(SANI_W1_WR);//данные на запись
	    switch (data_wr_w1) {
		case 0xAA ://0xcc=204 0x44=68 , 0xcc=204 0xbe=190
		    *(unsigned char *)SANI_W1_RD_DONE = '0';
		    rd_cnt = 0; rd_uk = 0;
		    rst_dev(2, 0);//0=moto 1=gps 2=ds18 3=dht11
		    memset(chaka, 0, 80); sprintf(chaka,"DS18  : reset begin...");
		    usleep(1000);
		    if (dev_present(2)) {//DS18  //0=moto 1=gps 2=ds18 3=dht11 4=ADC
			memset(wr_cmd, 0, 16);
			wr_cmd[0] = 0xCC; wr_cmd[1] = 0x44;
			cmd_wr_dev(2, wr_cmd, 2);
			sprintf(chaka+strlen(chaka)," device DS18 present, write 0x%X 0x%X\n",wr_cmd[0],wr_cmd[1]);
			gps_log(chaka); bug_log(chaka);
			//-----------------------------------------------------------
			usleep(1000);
			rd_cnt = 9; rd_uk = 0;
			rst_dev(2, 0);//0=moto 1=gps 2=ds18 3=dht11 4=ADC
			memset(chaka, 0, 80);
			sprintf(chaka,"DS18  : reset begin...");
			if (SANI_DS18) memset(SANI_DS18, 0xff, sizeof(s_ds18_temp));
			usleep(1000);
			if (dev_present(2)) {//DS18  //0=moto 1=gps 2=ds18 3=dht11 4=ADC
			    memset(wr_cmd, 0, 16);
			    wr_cmd[0] = 0xCC; wr_cmd[1] = 0xBE;
			    cmd_wr_dev(2, wr_cmd, 2);
			    sprintf(chaka+strlen(chaka)," device DS18 present, write 0x%X 0x%X\n",wr_cmd[0],wr_cmd[1]);
			    gps_log(chaka); bug_log(chaka);
			    ReadStart(2, rd_cnt);
			    ...
			    *(unsigned char *)SANI_W1_RD_DONE = '1';
			    sprintf(chaka+strlen(chaka)," no DS18 device.\n"); 
			    gps_log(chaka); bug_log(chaka);
			}
			//-----------------------------------------------------------
		    } else {
	    ...
	    //-------------------   DHT11   ------------------------------------
	    *(unsigned char *)SANI_W1_RD_DHT_DONE = '0';
	    rd_cnt_dht = 5; rd_uk_dht = 0; pst_dht = 0; memset(rd_dht, 0, 16); 
	    memset(SANI_W1_RD_DHT, 0, 8); //memset(SANI_DHT11,0xff,sizeof(s_dht11));
	    rst_dev(3, 0);//0=moto 1=gps 2=ds18 3=dht11 4=ADC
	    pst_dht = 1;
	    if (SANI_DHT11) memset(SANI_DHT11, 0xff, sizeof(s_dht11));
	    usleep(1000);
	    ReadStart(3, 5);//rd_cnt_dht
	    memset(chaka, 0, 80); sprintf(chaka,"DHT11 : reset+start begin, wait data ...\n"); gps_log(chaka); //bug_log(chaka);
	    //-------------------   ADC   --------------------------------------
	    if (SANI_ADC) memset(SANI_ADC, 0, sizeof(s_adc));
	    pst_adc = 1; rd_cnt_adc = 4; rd_uk_adc = 0;
	    ReadStart(4, sel_chan);
	    memset(chaka, 0, 80); sprintf(chaka,"ADC (%d) : start, wait data ...\n",sel_chan); gps_log(chaka); //bug_log(chaka);
	    //-------------------------------------------------------------------
	...
	if (!pst_hcsr) {//-------------------   HC-SR04   ----------------------------------
		...
		pst_hcsr = 1; rd_cnt_hcsr = 4; rd_uk_hcsr = 0; //data_hcsr=data_hcsr_n=-1;
		ReadStart(5, rd_cnt_hcsr);
		memset(chaka, 0, 80); sprintf(chaka,"HCSR : start, wait data ...\n"); gps_log(chaka); bug_log(chaka);
		...
		if (SANI_HCSR) {
		    memset(SANI_HCSR, 0xff, sizeof(int));
		    //data_hcsr_n=htonl(data_hcsr/58);
		}
		memset(chaka, 0, 80);
		sprintf(chaka,"HCSR  timeout.\n");
		bug_log(chaka); gps_log(chaka);
		pst_hcsr = 0;
		ReadStop(5);//HCSR
		tmr_hcsr = get_timer_msec(msec_def);
	    }
	...
	if (!pst_hcsr2) {//-------------------   HC-SR04-2   ----------------------------------
		...
		pst_hcsr2 = 1; rd_cnt_hcsr2 = 4; rd_uk_hcsr2 = 0; //data_hcsr2=data_hcsr2_n=-1;
		ReadStart(7, rd_cnt_hcsr2);
		memset(chaka, 0, 80); sprintf(chaka,"HCSR2 : start, wait data ...\n"); gps_log(chaka); bug_log(chaka);
		...
		if (SANI_HCSR2) {
		    memset(SANI_HCSR2, 0xff, sizeof(int));
		    //data_hcsr2_n=htonl(data_hcsr2/58);
		}
		memset(chaka, 0, 80);
		sprintf(chaka,"HCSR2 timeout.\n");
		bug_log(chaka); gps_log(chaka);
		pst_hcsr2 = 0;
		ReadStop(7);//HCSR2
		tmr_hcsr2 = get_timer_msec(msec_def);
	...
	if (!pst_enc) {//-------------------   ENC  ----------------------------------
		...
		pst_enc = 1;
		ReadStart(6, 6);//надо читать 6 байт
		tmr_enc = get_timer_msec(msec_enc);
		//-------------------------------------------------------------------
	    }
	}
	...
	if (fst) {
	    fst = 0; rst_dev(1, bt);//reset GPS : 1=gps,bt=0/1
	    ReadStart(1, msg_type_ind);// set msg_type to 0 - $GPMRC
	    if (SANI_MSG_TYPE) *(unsigned char *)SANI_MSG_TYPE = msg_type_ind;
	}

	if (check_delay_sec(tmrs)) { tmrs = get_timer_sec(2); if (!en) en = 1; }
	...
	if (select(my_max_fd+1, &my_read_Fds, NULL, NULL, &mytv) > 0) {
		...
		if (FD_ISSET(fd[id], &my_read_Fds)) {
			...
			case 1://from GPS
			    ...
			    lens = ReadRead(id, (unsigned char *)buf);//GPS //конец пакета 0x0d,0x0a
...
//				    if (convert) {
//					memset((unsigned char *)&web_pack.utc.chas, 0, sizeof(s_pack));
//					conver_pack(&web_pack);
//				    }
				    memcpy((unsigned char *)SANI_PACK, web_uk, sizeof(s_pack));
				    if (en) en = 0;
				}// else gps_log(buf);
			    }
			break;
			case 2://from DS18
				...
				if (rd_uk > 0) {
				    ...
				    byte = rd_uk;	byte &= 0x0f;
				    *(unsigned char *)SANI_W1_RD = byte;//количество прочитанных байт
			...
			case 3://from DHT11
				...
				if (rd_uk_dht > 0) {
				    DHT11_ALL(rd_dht);
				    sprintf(dht11_str,"[%d,%d%% +%d,%d%cC]",dht11.rh_cel,dht11.rh_dro,dht11.tp_cel,dht11.tp_dro,0x9c);
				    byte = rd_uk_dht; byte &= 0x07;
				    *(unsigned char *)SANI_W1_RD_DHT = byte;//количество прочитанных байт
				    memcpy(SANI_W1_RD_DHT+1, rd_dht, byte);
				    *(unsigned char *)SANI_W1_RD_DHT_DONE = '1';
			...
			case 5://HC-SR04
				...
				if (SANI_HCSR) {
				    ...
				    memcpy(SANI_HCSR, (unsigned char *)&data_hcsr_n, sizeof(int));
				}
				memset(chaka, 0, 80);
			...
			case 6://ENC
			    if (pst_enc) {
				pst_enc = 0; memset(rd_enc, 0, 16); memset(chaka, 0, 80);
				memset(SANI_ENC_WAY, 0, sizeof(s_enc));
				rd_cnt_enc = ReadRead(id, rd_enc);//ENC
				if (rd_cnt_enc == 6) {
				    memcpy(&enc_int, rd_enc, 4); memcpy(&enc_short, rd_enc+4, 2);
				    enc.way = htonl(enc_int); enc.speed = htons(enc_short);
				    memcpy(SANI_ENC_WAY, (unsigned char *)&enc.way, sizeof(s_enc));
				    ...
				    sprintf(chaka,"ENC [way=%.02f m (0x%08X) (%d sm), speed=%.02f m/sec (0x%04X)]\n",
						enc_int_float,enc_int,enc_way_sm,
						enc_short_float,enc_short);
			...
			case 7://HC-SR04-2
				...
				if (SANI_HCSR2) {
				    ...
				    memcpy(SANI_HCSR2, (unsigned char *)&data_hcsr2_n, sizeof(int));
				}
				memset(chaka, 0, 80);
	...
	if (check_delay_sec(tmr_w1)) {
		...
		*(unsigned char *)SANI_W1_RD_DONE = '1';
		...
		if (rd_uk == rd_cnt) sprintf(chaka,"DS18 %s (%d) timeout : ",ds18_str, rd_uk);//прочитано 9 байт
			        else { sprintf(chaka,"DS18 %s (%d) timeout : ",ds18_str, rd_uk); rd_cnt = rd_uk; }
		rd_cnt &= 0x0F;
		if (rd_cnt > 0) {
		    for (i = 0; i < rd_cnt; i++) sprintf(chaka+strlen(chaka),"0x%X ",rd_cmd[i]);
		}
		sprintf(chaka+strlen(chaka),"\n");
		gps_log(chaka); bug_log(chaka);
	    } else *(unsigned char *)SANI_W1_RD_DONE = '1';
	    pst = 0;
	    ReadStop(2);//DS18
		...
		*(unsigned char *)SANI_W1_RD_DHT_DONE = '1';
		...
		if (rd_uk_dht == rd_cnt_dht) sprintf(chaka,"DHT11 %s (%d) timeout : ",dht11_str, rd_uk_dht);//прочитано 5 байт
			              else { sprintf(chaka,"DHT11 %s (%d) timeout : ",dht11_str, rd_uk_dht); rd_cnt_dht = rd_uk_dht; }
		rd_cnt_dht &= 0x0F;
		if (rd_cnt_dht > 0) {
		    for (i = 0; i < rd_cnt_dht; i++) sprintf(chaka+strlen(chaka),"0x%X ",rd_dht[i]);
		}
		sprintf(chaka+strlen(chaka),"\n");
		gps_log(chaka); bug_log(chaka);
	    } else *(unsigned char *)SANI_W1_RD_DHT_DONE = '1';
	    pst_dht = 0;
	    ReadStop(3);//DHT11
	...
	N_RK_IN = *(unsigned char *)SANI_RK & 3;//max 4 channel
	if (N_RK != N_RK_IN) { N_RK = N_RK_IN; *(unsigned char *)SANI_RK = N_RK; }
	...
	for (i = 0; i < all_radio; i++) {
		...
		switch (gsm_rk[i].faza) {
			...
			if (check_delay_msec(tmr1[i])) {
			    gsm_rk[i].cmd = 0; gsm_rk[i].cmd_ind = 0;
			    gsm_rk[i].err_main = 0; gsm_rk[i].err = 0; gsm_rk[i].err_count = 0;
			    GSMStartStop(i, (unsigned char)SET_GSM_START);
			    ModulePWR_ON(i);
			...
			if ((gsm_rk[i].block) || (!loops) || (gsm_rk[i].err_main>=max_err_main)) {
				...
				if (gsm_rk[i].cmd) {//если канал свободен
					...
					gsm_rk[i].cmd = 0; gsm_rk[i].answer = 0; gsm_rk[i].idle = 0; //RK_BUSY;//0;
					gsm_rk[i].err_count = 0; gsm_rk[i].err = 0; gsm_rk[i].answer = 0;
					memset(stz, 0, 128); strcpy(stz, ATCOM.com_); put_AT_com(stz, i);
					memset(ATCOM.txt_, 0, 200);
			...
			if ((!check_vio(i)) || (check_delay_msec(tmr1[i]))) {
			    ...
			    gsm_rk[i].rx_faza = 0;
			    gsm_rk[i].tx_faza = 0;
			    if (!loops) stop_all_rk |= ((unsigned char)1 << i);
			}
		    break;
		}
		...
		switch (gsm_rk[i].rx_faza) {
			...
			if (strstr((char *)&gsm_rk[i].RX[0],"PB DONE") != NULL) {
			    gsm_rk[i].cmd = 1; gsm_rk[i].err = 0;
			} else 
			if (strstr((char *)&gsm_rk[i].RX[0],"+CPIN: READY") != NULL) {
			    if (RK_table[i].type != 2) {
				gsm_rk[i].cmd = 1; gsm_rk[i].err = 0;
			    }
			} else if (strstr((char *)&gsm_rk[i].RX[0],"OK") != NULL) {
			    gsm_rk[i].cmd = 1; gsm_rk[i].err = 0; gsm_rk[i].answer = 1;
			} else if (strstr((char *)&gsm_rk[i].RX[0],"ERR") != NULL) {
			    gsm_rk[i].cmd = 1; gsm_rk[i].err = 1; gsm_rk[i].answer = 1;
			    if (strstr((char *)&gsm_rk[i].RX[0],"SIM failure")!=NULL) {
				gsm_rk[i].answer = 0; gsm_rk[i].err_main = max_err_main;
			    }
			...
			if (gsm_rk[i].answer) {
			    if (ATCOM.flag_) {
				ATCOM.flag_ = 0;
				send_to_web((unsigned char *)&ATCOM.txt_[0], ATCOM.dl_);
				gsm_rk[i].err = 0; gsm_rk[i].err_count = 0;
				tmr1[i] = get_timer_msec(_1s);
			    } else {
				if (gsm_rk[i].err) {
				    gsm_rk[i].err_count++;
				    if (gsm_rk[i].err_count > 2) {
					gsm_rk[i].err = 0; gsm_rk[i].err_count = 0; gsm_rk[i].cmd_ind++;
			...
			gsm_rk[i].data = 0;
		    break;
			default : { gsm_rk[i].data = 0; gsm_rk[i].rx_faza = 1; }
		}
		
		switch (gsm_rk[i].tx_faza) {
		    case 0 : break;
		    case 1 :
			//gsm_rk[i].StatusAdp=0; //статус СОМ-порта радиоканала в исх. состояние
...
//#define ModOnOff    2       //  управление вкл.выкл модуля (конт. ON/OFF)
	...
	if (stop_all_rk >= 0x0f) {
	    memset(chaka, 0, 80); sprintf(chaka,"All channels shutdown.\n"); print_rk(chaka);
	    break;
	} else if (check_delay_sec(tmrs)) {
	    memset(chaka, 0, 80); sprintf(chaka,"All channels shutdown by timer.\n"); print_rk(chaka);
	    break;
	}
    }
//---------------------------------------------------------------------------------
